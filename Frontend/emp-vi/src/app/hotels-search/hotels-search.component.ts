...
export class HotelsSearchComponent implements OnInit {
  ...
  'Tesla',
  'Uber'];
  options2: string[] = ['CapGemini','Infosys'];
  ...
  search() {
    ...
    this.hotelSearchRequest = new HotelSearchRequest(this.sourceControl.value, "");
    this.hotelSearchService.getProjects(this.sourceControl.value).subscribe(response => {
